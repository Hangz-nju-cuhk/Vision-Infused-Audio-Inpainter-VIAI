...
class WaveNet(nn.Module):
    ...
    def __init__(self, out_channels=hparams.out_channels, layers=hparams.decode_layers, stacks=hparams.decode_stacks,
                 residual_channels=hparams.residual_channels,
                 gate_channels=hparams.gate_channels,
                 skip_out_channels=hparams.skip_out_channels,
                 kernel_size=hparams.kernel_size, dropout=hparams.dropout,
                 cin_channels=hparams.cin_channels, gin_channels=hparams.gin_channels, n_speakers=hparams.n_speakers,
    ...
    def forward(self, x, c=None, g=None, softmax=False):
        ...
        x = self.softmax(x, dim=1) if softmax else x
    ...
    # def forward(self, x, c=None, g=None, softmax=False, is_incremental=False, initial_input=None,
    #                         T=100, test_inputs=None, tqdm=lambda x: x, quantize=True, log_scale_min=-7.0):
    #     if not is_incremental:
    #         self._forward(x, c, g, softmax)
    #
    #     else:
    #         self.incremental_forward(initial_input, c, g,
    #                         T, test_inputs,
    #                         tqdm=tqdm, softmax=softmax, quantize=quantize,
