...
def shift_right(x):
    ...
    Args:
      x: The [mb, channels, time] tensor input.

    Returns:
      x_sliced: The [mb, channels, time] tensor output.
    """
    padding = ConstantPad2d((1, 0, 0, 0), 0)
    x_padded = padding(x)
    x_sliced = x_padded[:, :, :-1]
    return x_sliced


def Conv1d(in_channels, out_channels, kernel_size=1, padding=0, dilation=1, bias=True, weight_normalization=True,
           dropout=0, std_mul=1.0, **kwargs):
    m = conv.Conv1d(in_channels, out_channels, kernel_size=kernel_size, padding=padding, dilation=dilation, bias=bias, **kwargs)
...
class EncodeConv(nn.Module):
    """ReLU + Residual dilated conv1d + ReLU + conv1x1
    ...
    def __init__(self, residual_channels, gate_out_channels=128, kernel_size=3, skip_out_channels=None,
        ...
        if weight_normalization:
            assert bias
            self.conv = Conv1d(residual_channels, gate_out_channels, kernel_size,
                               padding=padding, dilation=dilation, weight_normalization=True,
                               bias=bias, std_mul=1.0, *args, **kwargs)
        else:
            self.conv = Conv1d(residual_channels, gate_out_channels, kernel_size,
                                    padding=padding, dilation=dilation,
                               weight_normalization=False, bias=bias, *args, **kwargs)

        self.conv1x1_out = Conv1d1x1(gate_out_channels, skip_out_channels, bias=bias,
                                     weight_normalization=weight_normalization)

    def forward(self, x, is_incremental=False):
        """Forward
